...
- GUI install from npm in n8n (`Settings > Community Nodes`) for unverified/self-hosted usage
- Verified community node install from nodes panel (if verified and eligible)
...
If operations support compatibility fallbacks (for API route changes across upstream versions), document the fallback order explicitly.
...
- self-hosted vs cloud limitations, including verified vs unverified install behavior
...
- endpoint/version mismatch behavior and route fallback expectations
